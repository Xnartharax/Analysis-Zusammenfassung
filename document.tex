...
\begin{definition}[messbare Abbildung]
	Für eine messbare Abbildung $f: (\Omega, \mathcal{A}) \to (\tilde{\Omega}, \tilde{\mathcal{A}})$ gilt: $f^{-1}(A) \in \mathcal{A} \forall A \in \tilde{\mathcal{A}}$.
\end{definition}
\begin{theorem}
	...
	$$f^{-1}(E) \in \mathcal{A} \forall E \in \mathcal{E}$$
\end{theorem}
\begin{theorem}
...
\begin{definition}[Wesentliches Supremum]
	...
	Außerdem seien $\mathcal{L}:=\{f|\text{esssup}(f)< \infty\}$ und $L^\infty = \mathcal{L}^\infty/\sim$.
\end{definition}
\begin{theorem}
...
\begin{theorem}[Transformationsformel]
	...
	$$\int_\Omega fd\Phi(\mu) = \int_{\tilde{\Omega}}f\circ \Phi d\mu$$
\end{theorem}
...
\begin{definition}[Trigonometrisches Polynom]
	...
	Mit $$a_0 = 2c_0, a_k = c_k +c_{-k}, b_k = i(c_k -c_{-k})$$.
...
\begin{definition}[Dirichlet-Kern]
	...
	$$D_N(x) = \begin{cases}
		...
		\frac{1}{2\pi}\frac{\sin((2N+1)x/2)}{\sin(x/2)} & \text{andernfalls}
	\end{cases}$$
\end{definition}
